...
class MyFirstModel(DynamicModel):
    def __init__(self, melt, atmospericLoss, seepage, templap, infil):
        DynamicModel.__init__(self)
        setclone('clone.map')

        # assign 'external' input to the model variable
        self.meltRateParameter = melt 
        self.atmosphericLoss = atmospericLoss

        # proportion of subsurface water that seeps out to surface water per day
        self.seepageProportion =  seepage 
        self.temperatureLapseRate = templap
        self.infiltrationCapacity = infil
    ...
    def dynamic(self):
        ...
        self.difference = discharge - dis_obs
        #self.report(self.difference , "dMmo")
        #self.report(dis_obs, "dO")
